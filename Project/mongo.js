...
MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true }, function(err, database) {
    var dbo = database.db(dbName);
    var arr = [];
    fs.readdir('./public/images/Chapters/DragonBallZ/Chap1', 'utf-8', (err, files) => {
        if (err) {
            console.log(err);
            return;
        } else {
        ...
        for (var i = 0; i < arr.length; i++) {
            temp[i] = 'images/Chapters/DragonBallZ/Chap1/' + arr[i];
        }
        dbo.collection('manga').update({ name: 'Dragon Ball Z' }, {
            $push: {
                chapter: { $each: temp }
            }
        })
